...
c====== Variables for code ========
      ...
      double precision psii(boxsize*res,boxsize*res)
      double precision psi(boxsize*res,boxsize*res)
      double precision a(waves)
      double precision b(waves)
      double precision theta, thetai
      double precision delta, deltai
...
c====== Output initialization =====
      open(unit = 2, file = 'pnC_L512_w1000_res1_iter10.dat')
      open(unit = 3, file = 'pnC_L512_w1000_res1_iter10_maxima.dat')
      open(unit = 4, file = 'pnC_L512_w1000_iter10_maximaFULL.dat')
...
c========= Initialize the arrays ==
         ...
         call normalen(waves,b)
c========= Generate the waves ===
         do i = 1, (waves)
            call zufall(4,rands)
            theta = rands(1)*2*3.14159256
            thetai = rands(2)*2*3.14159256
            delta = rands(3)*2*3.14159256
            deltai = rands(4)*2*3.14159256
            do xi = 1, (boxsize*res)
               do yi = 1, (boxsize*res)
                  psir(xi,yi) = psir(xi,yi) +
     $            a(i)*cos((xi)*dres*cos(theta)/wavelength +
     $            (yi)*dres*sin(theta)/wavelength + delta)
                  psii(xi,yi) = psii(xi,yi) +
     $            b(i)*sin((xi)*dres*cos(thetai)/wavelength +
     $            (yi)*dres*sin(thetai)/wavelength + deltai)
               enddo
            enddo
         enddo
